...
function prepareQuery(ast, typeKey, exclude, getNodeName) {
...
export class Parser {
  constructor(id, options) {
    if (typeof id === 'object') {
      ...
      id = glob
        .sync(`${__dirname}/*`, { ignore: './node_modules/*' })
        .find((name) => {
    ...
    if (options.filename && this.category.decideParser) {
      this.parser = this.getParserByID(
        this.category.decideParser(options.filename)
      );
    } else {
      this.parser =
        this.getParserByID(options.parser) ||
        this.getDefaultParser(this.category);
    }
    this.parser.loadParser((parser) => {
      this.parse = (source, options = this.parser.getDefaultOptions()) => {
        if (!this.cache[source]) {
          try {
            const ast = this.parser.parse(parser, source, options);
            const visitorKeys = prepareQuery(
              ast,
  ...
  loadCategory(catName) {
    ...
    category.codeExample = interopRequire(
      localRequire(`./${catName}/codeExample.txt`)
    );

    const catFiles = glob
      .sync(`./${catName}/*.js`, { cwd: __dirname })
      .map((name) => name.split('/').slice(2));
    category.parsers = catFiles
      .filter(([parserName]) => !restrictedParserNamesSet.has(parserName))
      .map(([parserName]) => {
        const parser = interopRequire(
          localRequire(`./${catName}/${parserName}`)
        );
