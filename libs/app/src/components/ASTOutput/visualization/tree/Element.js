...
import {useSandpack} from '@gencodes/app/hooks/useSandpack';
...
const Element = React.memo(function Element({
  ...
  let range = [0,0]
  try{
    range = treeAdapter.getRange(value);
    ...
    (state, own) => {
      if (own) {
        if (state === OPEN_STATES.CLOSED) {
          sandpack.setSelectedNode(null);
          setSelected(false);
        } else {
          sandpack.setHighlight(props.treeAdapter.getRange(props.value));
          setSelected(true);
        }
      }
      if (props.onClick) {
