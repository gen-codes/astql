...
export interface SandpackProviderState {
  queries: SandpackBundlerFiles;
  files: SandpackBundlerFiles;
  environment?: SandboxEnvironment;
  activeQueryPath: string;
  activePath: string;
  openPaths: string[];
  openQueryPaths: string[];a
  startRoute?: string;
...
> {
  ...
  updateCurrentQuery = (newCode: string): void => {
    this.updateQuery(this.state.activeQueryPath, newCode);
  };
  updateCurrentFile = (newCode: string): void => {
    this.updateFile(this.state.activePath, newCode);
  };
  updateQuery = (path: string, newCode: string): void => {
    if (newCode === this.state.queries[this.state.activeQueryPath]?.code) {
      return;
    }
    ...
    this.setState({ queries: newQueries }, this.updateClients);
  };
  createQuery = (path: string): void => {
    if (this.state.queries[path]) {
      return;
    }
  ...
  componentDidUpdate(prevProps: SandpackProviderProps): void {
    if (
      ...
      const { activePath, openPaths, files, environment,queries } =
        getSandpackStateFromProps(this.props);
      
      /* eslint-disable react/no-did-update-set-state */
      this.setState({ activePath, openPaths, files, environment, queries });
  ...
  setActiveFile = (path: string): void => {
    this.setState({ activePath: path, editorState: 'dirty', highlight: null });
  };
  setActiveQuery = (path: string): void => {
    this.setState({ activeQueryPath: path });
  };
  ...
  openQuery = (path: string): void => {
    this.setState(({ openQueryPaths }) => {
      const newPaths = openQueryPaths.includes(path)
        ? openQueryPaths
        : [...openQueryPaths, path];

      return {
        activeQueryPath: path,
        openQueryPaths: newPaths,
      };
    });
  };

  closeQuery = (path: string): void => {
    if (this.state.openQueryPaths.length === 1) {
      return;
    }

    this.setState(({ openQueryPaths, activePath }) => {
      const indexOfRemovedPath = openQueryPaths.indexOf(path);
      const newPaths = openQueryPaths.filter((openPath) => openPath !== path);
      ...
      return {
        activePath:
          path === activePath
            ? indexOfRemovedPath === 0
              ? openQueryPaths[1]
              : openQueryPaths[indexOfRemovedPath - 1]
            : activePath,
        openQueryPaths: newPaths,
      };
    });
  ...
  deleteQuery = (path: string): void => {
    this.setState(({ openQueryPaths, files }) => {
      const newPaths = openQueryPaths.filter((openPath) => openPath !== path);
      const newFiles = Object.keys(files).reduce(
        (acc: SandpackBundlerFiles, filePath) => {
          if (filePath === path) {
  ...
  resetQuery = (path: string): void => {
    const { queries } = getSandpackStateFromProps(this.props);

    this.setState((prevState) => ({
      queries: { ...prevState.queries, [path]: queries[path] },
    }));
  };
  ...
  _getSandpackState = (): SandpackContext => {
    ...
    return {
      ...
      resetAllQueries: this.resetAllQueries,
