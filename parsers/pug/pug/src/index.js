  ...
  getNodeName(node) {
    ...
    switch (type) {
      case 'Block':
        return '';
      case 'Doctype':
        return `Doctype(${node.val})`;
      case 'Comment':
        if (node.buffer) return 'Comment(buffer)';
      case 'NamedBlock':
        return `Block:${node.mode}(${node.name})`;
      case 'Code':
        if (node.val === 'break') return 'Code(break)';
      case 'When':
        if (node.expr === 'default') return 'When(default)';
      case 'Include':
      case 'RawInclude':
      case 'Extends':
      case 'Each':
