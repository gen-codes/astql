...
export default {
  ...
  loadParser(callback) {
    new Promise((resolve)=> resolve(['regexp-tree'].map((mdl)=>require(mdl)))).then( (regexpTree) => {
      callback(regexpTree);
    });
  },

  parse(regexpTree, code, options = {}) {
    regexpTree.parser.setOptions(options);
  ...
  opensByDefault(node, key) {
    return node.type === 'RegExp' || key === 'body' || key === 'expressions';
  },
